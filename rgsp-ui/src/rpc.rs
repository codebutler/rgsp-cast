...
impl Control {
    ...
    ///
    /// Blocking, deliberately: an earlier version of this method was split
    /// into a non-blocking `start_submit_pin`/`poll_submit_pin` pair so a
    /// "Pairing…" screen could offer a genuine `B Cancel` while waiting.
    /// The owner ruled that out — no button is more honest than a disabled
    /// one, and a blocking call plus a message drawn before it (see
    /// `main.rs`'s `PinAction::Submit` handling) is simpler than carrying a
    /// second concurrency model for a button nobody asked to keep.
